<?php
    class M_GooProper extends CI_Model{
        ...
        function DetailPrimary($idListing){
            $this->db->select('tipenewlisting.*, listingnew.*');
            $this->db->from('tipenewlisting');
            $this->db->where('tipenewlisting.IdListing', $idListing);
            $this->db->join('listingnew', 'tipenewlisting.IdNew = listingnew.IdNew', 'left');
            $result = $this->db->get();
            if($result->num_rows()>0){
                return $result->result();
            }else {
